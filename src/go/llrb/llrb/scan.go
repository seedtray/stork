...
//Iterator is the interface for all scan/traversal methods in Smap.
...
func (m *Smap) Scan() *ScannerBuilder {
...
func (sb *ScannerBuilder) From(k Key) *ScannerBuilder {
	sb.left = &Edge{k, true}
	return sb
}

//After specifies that the scan starts from a given key and that the range is left-open.
func (sb *ScannerBuilder) After(k Key) *ScannerBuilder {
	sb.left = &Edge{k, false}
	return sb
}

//To specifies that the scan stops at a given key and that the range is right-closed.
func (sb *ScannerBuilder) To(k Key) *ScannerBuilder {
	sb.right = &Edge{k, true}
	return sb
}

//Before specifies that the scan stops at a given key and that the range is right-open.
func (sb *ScannerBuilder) Before(k Key) *ScannerBuilder {
	sb.right = &Edge{k, false}
	return sb
}
...
func (sb *ScannerBuilder) Start() Iterator {
	m := sb.m
...
//FromScan returns an iterator that scans through the Smap keys in order starting at the specified edge.
func (m *Smap) FromScan(from Edge) Iterator {
	stack := m.buildLeftBoundStack(from)
	return &Scanner{stack, nil, false}
}

//FullScan returns an iterator that scans through all the Smap keys in order
func (m *Smap) FullScan() Iterator {
...
//FromScan returns an iterator that scans through the Smap keys in order stopping at the specified edge.
func (m *Smap) UpToScan(to Edge) Iterator {
	boundary := m.getRightBound(to)
	scanner := m.FullScan().(*Scanner)
	return &upToScanner{scanner, boundary, false}
}

//RangeScan returns an iterator that scans through the Smap keys in order between the given start and end edges.
func (m *Smap) RangeScan(from, to Edge) Iterator {
	boundary := m.getRightBound(to)
...
func (m *Smap) makeHeightStack() *fixedNodeStack {
...
func (m *Smap) buildLeftBoundStack(left Edge) *fixedNodeStack {
	stack := m.makeHeightStack()
	key := left.Key
...
//find the right boundary in the Smap given a right edge.
//The returning node will have the greater key
//such that node.Key <= right.Key (< for open edge).
//The edge key does not need to be in the Smap
func (m *Smap) getRightBound(right Edge) *Node {
	var last *Node = nil
	key := right.Key
	if right.Key == nil {
		return nil
